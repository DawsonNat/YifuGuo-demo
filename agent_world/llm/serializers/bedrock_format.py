...
class AWSBedrockMessageSerializer:
    ...
    def _download_and_convert_image(url: str) -> tuple[str, bytes]:
        ...
        try:
            ...
            raise ImportError(
                "httpx not available. Please install it to use URL images with AWS Bedrock."
            ) from e
        ...
        for part in content:
            if part.type == "text":
                content_blocks.append(
                    AWSBedrockMessageSerializer._serialize_content_part_text(part)
                )
        ...
        if isinstance(message, UserMessage):
            ...
            if message.content is not None:
                content_blocks.extend(
                    AWSBedrockMessageSerializer._serialize_assistant_content(message.content)
                )
            ...
            if message.tool_calls:
                for tool_call in message.tool_calls:
                    content_blocks.append(
                        AWSBedrockMessageSerializer._serialize_tool_call(tool_call)
                    )
    ...
    def serialize_messages(
        ...
        for message in messages:
            if isinstance(message, SystemMessage):
                ...
                system_message = AWSBedrockMessageSerializer._serialize_system_content(
                    message.content
                )
            else:
