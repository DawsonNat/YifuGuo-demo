...
class ResponsesAPIMessageSerializer:
        ...
        for part in content:
            if part.type == "text":
                serialized_parts.append(
                    ResponsesAPIMessageSerializer._serialize_content_part_text(part)
                )
            elif part.type == "image_url":
                serialized_parts.append(
                    ResponsesAPIMessageSerializer._serialize_content_part_image(part)
                )
    ...
    def _serialize_assistant_content(
        ...
        for part in content:
            ...
            # Refusals are converted to text for the Responses API
                ...
                serialized_parts.append(
                    ResponseInputTextParam(text=f"[Refusal: {part.refusal}]", type="input_text")
                )
        return serialized_parts
    ...
    @staticmethod
    def serialize_messages(messages: Sequence[BaseMessage]) -> list[EasyInputMessageParam]:
