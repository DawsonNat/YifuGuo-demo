...
class OllamaMessageSerializer:
        ...
        for tool_call in tool_calls:
            ...
            ollama_tool_call = Message.ToolCall(
                function=Message.ToolCall.Function(
                    name=tool_call.function.name, arguments=arguments_dict
                )
            )
    ...
    def serialize(message: BaseMessage) -> Message:
        ...
        if isinstance(message, UserMessage):
            ...
            if message.tool_calls:
                ollama_message.tool_calls = OllamaMessageSerializer._serialize_tool_calls(
                    message.tool_calls
                )
    ...
    @staticmethod
    def serialize_messages(messages: Sequence[BaseMessage]) -> list[Message]:
        """Serialize a list of BaseMessage to Ollama Messages."""
